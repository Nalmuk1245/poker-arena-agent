...
class BankrollManager {
    ...
    minRiskPerGame;
    stopLossThreshold;
    constructor(initialBankroll, options) {
     ...
     */
    getOptimalWager(winProbability, potOdds = 2.0) {
        ...
        let riskLevel;
        if (bankrollRatio > 0.8)
            riskLevel = "LOW";
        else if (bankrollRatio > 0.6)
            riskLevel = "MEDIUM";
        else if (bankrollRatio > this.stopLossThreshold)
            riskLevel = "HIGH";
        else
            riskLevel = "CRITICAL";
    ...
    getRiskLevel() {
        ...
        if (ratio > 0.6)
            return "MEDIUM";
        if (ratio > this.stopLossThreshold)
            return "HIGH";
        return "CRITICAL";
    }
}
